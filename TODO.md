...
- [x] 2D에서 무반응 스크린샷 버튼 비활성화
- [x] 3D 전용 PNG 저장 제한 이유 안내
- [x] 3D 복귀 시 활성화와 실제 PNG 다운로드 검증

### M40 제품 아키텍처·워크플로우 문서 ✅

- [x] `docs/products/` 개요·아키텍처·워크플로우 HTML 문서 작성
- [x] 도메인·애플리케이션·인프라·프레젠테이션 경계와 런타임 조립 시각화
- [x] 도면→3D, Attach/Detach, 견적, catalog import, 저장 왕복, GitHub→Vercel 흐름 정리
- [x] 로컬 저장/동기화, CV 정확도, catalog coverage, 사용자 검증 공백 명시
- [x] 데스크톱·모바일 렌더링, overflow·console·링크 검증
- [x] README 영문·국문 주장과 저장소 아키텍처 드리프트 교정

판정: 문서 산출물은 구현 근거와 일치하도록 갱신했다. 다음 개발 범위는 새 기능 목록이 아니라 M23 실제 사용자 관찰에서 반복 재현되는 문제만 P0~P3로 분류해 선정한다.

### M41 README 언어 분리 + 30초 실제 사용 GIF ✅

- [x] 기본 `README.md` 영어 전용 재구성, `README.ko.md` 한국어 전용 분리
- [x] 양쪽 README에서 제품 개요·아키텍처·워크플로우 HTML 연결
- [x] 현재 검수 게이트를 통과하는 실제 Chromium 캡처 흐름으로 갱신
- [x] KIVIK 배치·실시간 가격·워크스루·점프를 포함한 30초 GIF 생성
