...
## 📌 향후 권장 (우선순위)
1. Vercel 배포 URL 확인 (대시보드 Import 후 — 스모크 예정)
2. AI 전체 해석 실측 (Ox Alpha 혼잡 해제 후 재실행)
3. ESLint·Prettier 도입 (Baseline 기본값 — 현재 미도입)
4. 실도면 추가 수집 (Wikimedia 429 해제 후 cv-benchmark 재실행)
5. 백로그: 공유 링크(읽기전용 뷰) · 다중선택 그룹 이동 · 접이식 가구 · DB 어댑터(계정 CRUD)
