...
`workers/triposr-worker` 자체를 Vercel Python Function으로 배포하지 않는다. 이 worker는 CUDA PyTorch 기반 이미지, Git 빌드형 `torchmcubes`, 네이티브 `xatlas/moderngl`, 지속 모델 캐시가 필요한 GPU Docker 서비스다. Vercel이 예약한 `services/` 아래에 두면 Python builder가 이 구성을 별도 serverless 서비스로 자동 감지하므로 지원 경로가 아니다. 루트 `.vercelignore`는 `workers/**`를 제외하고 `vercel.json`은 단일 Vite frontend build만 선언한다. `npm run test:vercel-boundary`가 두 경계를 함께 검사한다. worker는 아래 Docker 명령으로 GPU 호스트에 독립 배포한다.
