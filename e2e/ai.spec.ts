...
async function makePlanPng(page: import('@playwright/test').Page): Promise<Buffer> {
  const dataUrl = await page.evaluate(() => {
    ...
    g.lineWidth = 3
    g.beginPath(); g.moveTo(362, 40); g.lineTo(362, 362); g.stroke()
    g.beginPath(); g.moveTo(40, 362); g.lineTo(362, 362); g.stroke()
    g.beginPath(); g.moveTo(362, 240); g.lineTo(504, 240); g.stroke()
    g.beginPath(); g.moveTo(504, 40); g.lineTo(504, 240); g.stroke()
    g.fillStyle = '#111'
...
test('무료 vision 모델로 도면 해석 전체 플로우 (한도 소진 시 skip)', async ({ page }) => {
  const status = await openModalWithPlan(page, FREE_VISION_MODEL)
