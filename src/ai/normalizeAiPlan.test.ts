// TDD RED — 아직 구현되지 않은 normalizeAiPlan 모듈에 대한 테스트 (먼저 작성)
import { describe, it, expect } from 'vitest'
import { normalizeAiPlan } from './normalizePlan'
...
describe('normalizeAiPlan', () => {
  ...
  it('개구부 누락 치수를 타입 기본값으로 채운다', () => {
    const r = normalizeAiPlan({
      walls: [{ id: 'w1', a: { x: 0, y: 0 }, b: { x: 5000, y: 0 } }],
      openings: [{ wallId: 'w1', type: 'door' }, { wallId: 'w1', type: 'window' }],
    })
  ...
  it('꼭짓점 3개 미만 방은 제거하고 이름/ID를 정규화한다', () => {
    const r = normalizeAiPlan({
      walls: [{ id: 'w1', a: { x: 0, y: 0 }, b: { x: 5000, y: 0 } }],
      rooms: [
        { name: '안방', polygon: [{ x: 0, y: 0 }, { x: 4000, y: 0 }, { x: 4000, y: 3000 }, { x: 0, y: 3000 }] },
        { name: '깨진방', polygon: [{ x: 0, y: 0 }, { x: 100, y: 100 }] },
        { polygon: [{ x: 0, y: 0 }, { x: 500, y: 0 }, { x: 500, y: 500 }] }, // 이름 없음
      ],
    })
  ...
  it('wallHeight 기본값은 2400이고 숫자로 강제된다', () => {
    ...
    const r2 = normalizeAiPlan({ wallHeight: '2600', walls: [{ id: 'w1', a: { x: 0, y: 0 }, b: { x: 10, y: 0 } }] })
