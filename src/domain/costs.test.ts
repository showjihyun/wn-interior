...
describe('buildCostReport (배치 제품 가격 합산)', () => {
  ...
  it('카탈로그에서 찾지 못한 배치는 조용히 누락하지 않는다', () => {
    const r = buildCostReport([P('missing-placement', 'missing-product')], productOf)

    expect(r.unresolvedProducts).toEqual([{ productId: 'missing-product', qty: 1 }])
  })
})
