// TDD RED — 배치 단위 치수 오버라이드 (유사 제품 + 실측 조정 워크플로우)
...
describe('resolveDims (치수 오버라이드 해석)', () => {
  it('오버라이드 없으면 제품 실측 그대로', () => {
    ...
    expect(resolveDims(sofa, { ...pl, dimsOverride: undefined })).toEqual({ w: 2100, d: 950, h: 850 })
  })
  ...
  it('오버라이드 일부 누락/무효는 제품 값으로 폴백한다', () => {
    expect(resolveDims(sofa, { ...pl, dimsOverride: { w: 1800 } as any })).toEqual({ w: 1800, d: 950, h: 850 })
    expect(resolveDims(sofa, { ...pl, dimsOverride: { w: -5, d: 900, h: 800 } })).toEqual({ w: 2100, d: 900, h: 800 })
  })
