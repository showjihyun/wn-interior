...
describe('평면도 축척 적용 게이트', () => {
  ...
  it('실측값도 추정 축척 확인도 없으면 적용을 차단한다', () => {
    ...
    expect(result.message).toContain('실측')
  })
  ...
  it('실측값을 입력하면 보정 배율과 적용 가능 상태를 반환한다', () => {
    ...
    expect(result.message).toContain('11,800')
  })
  ...
  it('사용자가 위험을 명시적으로 확인한 경우에만 추정 축척을 허용한다', () => {
    ...
    expect(result.canApply).toBe(true)
    expect(result.message).toContain('추정')
  })
})
...
describe('변환 결과 사전 검토', () => {
  ...
  it('축척 보정 폭이 2배를 넘으면 실측값 재확인 경고를 추가한다', () => {
    ...
    expect(issues.find((issue) => issue.id === 'large-scale-correction')?.message).toContain(
      '3.59배'
    )
  })
