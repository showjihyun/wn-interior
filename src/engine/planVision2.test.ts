// TDD RED - CV 전처리 개선 (소형 연결요소 제거 / Otsu / 모폴로지 클로징 / 직교 스냅)
import { describe, it, expect } from 'vitest'
import { removeSmallComponents, autoThresholdOtsu, morphClose, orthogonalizePolygon, type Gray } from './planVision'
...
describe('autoThresholdOtsu (자동 임계값)', () => {
  it('이봉 히스토그램에서 두 모드 사이를 선택한다', () => {
    ...
    for (let i = 0; i < 100; i++) {
      rgba[i * 4] = 30; rgba[i * 4 + 1] = 30; rgba[i * 4 + 2] = 30 // 어두움
    }
    for (let i = 100; i < 200; i++) {
      rgba[i * 4] = 220; rgba[i * 4 + 1] = 220; rgba[i * 4 + 2] = 220 // 밝음
    }
