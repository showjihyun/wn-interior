...
import { selectRaster2SeqRooms, type Raster2SeqResponse } from './raster2seqRooms'
...
const safeResponse = (): Raster2SeqResponse => ({
  sourceWidth: 200,
  sourceHeight: 100,
  safe: true,
  rooms: [
...
describe('selectRaster2SeqRooms', () => {
  ...
  it('snaps to wall surfaces instead of the wall centerline', () => {
    const response: Raster2SeqResponse = {
      sourceWidth: 100,
      sourceHeight: 100,
      safe: true,
      rooms: [
  ...
  const fallbackCases: Array<[string, Raster2SeqResponse]> = [
  ...
  it('fails closed when the server omits its safety decision', () => {
    const response = safeResponse() as Partial<Raster2SeqResponse>
    delete response.safe

    const result = selectRaster2SeqRooms(legacyPlan(), response as Raster2SeqResponse, {
      targetWidth: 100,
      targetHeight: 100,
    })
