...
export interface Raster2SeqRoomResponse {
  name?: string
  polygon: Array<{ x: number; y: number }>
}

export interface Raster2SeqResponse {
  rooms: Raster2SeqRoomResponse[]
  sourceWidth: number
  sourceHeight: number
  device?: string
  inferenceMs?: number
...
export interface Raster2SeqRoomSelection {
  ...
  diagnostics?: Raster2SeqResponse['diagnostics']
}
...
export function selectRaster2SeqRooms(
  ...
  if (response.error) return fallback(legacyPlan, 'server-error', response)
