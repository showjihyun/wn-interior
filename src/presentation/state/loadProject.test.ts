...
describe('loadProject 격리 (AI/CV 결과는 새 프로젝트로)', () => {
  it('외부 콘텐츠 로드 시 새 id로 저장되고 기존 프로젝트를 보존한다', () => {
    ...
    const origName = s.projects.find((project) => project.id === origId)!.name
    ...
    // 기존 프로젝트 보존
    expect(st.projects.find((project) => project.id === origId)?.name).toBe(origName)
    // 새 프로젝트도 저장됨
    expect(st.projects.find((project) => project.id === st.projectId)?.name).toBe('CV 도면 변환')
  })

  it('id가 포함된 프로젝트도 충돌 방지를 위해 새 id를 발급한다', () => {
    ...
    expect(useStore.getState().projectId).not.toBe('explicit-1')
    void before
  })
})
