...
} from '../../application/productDimensionVariants'
  ...
  return (
    <div className="inspector">
      ...
      {product.dimensionVariants && product.dimensionVariants.length > 0 && (
        <>
          <label className="lbl">사용 상태</label>
          <div className="rowbtns dimension-variants">
            {product.dimensionVariants.map((variant) => (
              <button
                key={variant.id}
                className={activeDimensionVariant?.id === variant.id ? 'on' : ''}
                title={`${variant.dims.w}×${variant.dims.d}×${variant.dims.h}mm`}
                onClick={() => {
                  const patch = placementPatchForDimensionVariant(product, variant.id)
                  if (patch) updatePlacement(placement.id, patch)
                }}
              >
                {variant.label}
              </button>
      ...
      <div className="row3">
        <input
          ...
          onChange={(e) =>
            relocatePlacement(placement.id, parseInt(e.target.value) || 0, placement.pos.z)
          }
        />
        <input
          type="number"
          ...
          onChange={(e) =>
            relocatePlacement(placement.id, placement.pos.x, parseInt(e.target.value) || 0)
          }
        />
      </div>
