...
export function computeConflicts(placements: Placement[], productOf: (id: string) => Product | undefined): Set<string> {
...
function FurnitureItem({
  ...
  function down(e: ThreeEvent<PointerEvent>) {
    ...
    if (!isMovingThis) originRef.current = { x: placement.pos.x, z: placement.pos.z, rotY: placement.rotY }
  ...
  return (
    ...
    >
      ...
      {(isSel || hovered) && (
        <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, product.mount === 'ceiling' ? 40 : 6, 0]}>
          <ringGeometry args={[r, r + 50, 48]} />
          <meshBasicMaterial color={isSel ? '#ffd54a' : '#9ecbff'} transparent opacity={0.95} side={THREE.DoubleSide} />
        </mesh>
      )}
      {conflicted && (
      ...
      {isMovingThis && (
        <Html position={[product.dims.w / 2 + 120, product.dims.h + 160, 0]} center zIndexRange={[16777271, 100]}>
          <div
...
function SelectionOutline({ w, h, d, hangsDown }: { w: number; h: number; d: number; hangsDown: boolean }) {
...
function Ghost({ plan }: { plan: FloorPlan }) {
  ...
  const posRef = useRef<{ x: number; z: number; rotY: number; ok: boolean } | null>(null)
  ...
  function track(e: ThreeEvent<PointerEvent>) {
    ...
    const s = resolveSnap(plan, product, hit.x, hit.y, posRef.current?.rotY ?? 0)
    posRef.current = { x: s.x, z: s.z, rotY: s.rotY, ok: !!roomAt(plan, s.x, s.z) }
    force((n) => n + 1)
  }
    ...
    <>
      ...
      <mesh
        ...
        onClick={(e) => {
          e.stopPropagation()
          if (posRef.current) {
            // 확정 전 유효성 검사: 방 밖/충돌 시 Toast + 배치 거부
            const r = canDropAt(plan, product, useStore.getState().placements, null, posRef.current.x, posRef.current.z, posRef.current.rotY, (pid) => useStore.getState().productById(pid))
            if (!r.ok) {
              useStore.getState().showToast(
                r.reason === 'out-of-room' ? '방 안에만 배치할 수 있어요' : '공간이 부족해 배치할 수 없어요',
              )
              return
            }
            addPlacement(pendingId, { x: posRef.current.x, z: posRef.current.z }, posRef.current.rotY)
            setPending(null)
            posRef.current = null
          }
        }}
      ...
      {posRef.current && (
        <group position={[posRef.current.x, product.mount === 'ceiling' ? plan.wallHeight : 0, posRef.current.z]} rotation={[0, (posRef.current.rotY * Math.PI) / 180, 0]}>
          <group scale={[1, 1, 1]}>
            <Shape kind={product.shape} p={product} c={product.colorways?.[0]} />
          </group>
          <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 8, 0]}>
            <planeGeometry args={[product.dims.w + 80, product.dims.d + 80]} />
            <meshBasicMaterial color={posRef.current.ok ? '#59d499' : '#ff6b6b'} transparent opacity={0.3} side={THREE.DoubleSide} />
          </mesh>
        </group>
      )}
    </>
  ...
  return (
    <group>
      {placements.map((pl) => {
        ...
        if (!prod) return null
        return (
          <FurnitureItem key={pl.id} placement={pl} product={prod} plan={plan} conflicted={conflicts.has(pl.id)} />
        )
      })}
      <Ghost plan={plan} />
    </group>
